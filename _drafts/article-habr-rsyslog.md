...
КДПВ: нагуглить красивую консольку со сложными командами

Требуется передавать лог-файлы на центральный сервер. При недоступности этого сервера не терять сообщения, а накапливать и передавать при его появлении в сети. Дополнительно: не требуется изменение конфигурации сервера при появлении новых лог-файлов, достаточно перенастройки клиента. Дополнительно: можно передавать содержимое всех лог-файлов с соответствующим шаблону именем, причём на сервере их содержимое будет сохраняться раздельно в файлы с таким же именем.
