...
- `PRI` - Priority. Вычисляется как `facility * 8 + severity`.
  - Facilicy(категория) принимает значения от 0 до 23, им соответствуют различные категории системных служб: 0 - kernel, 2 - mail, 7 - news. Последние 8 - от local0 до local7 - определены для служб, не попадающих в предопределённые категории. [Полный список](https://en.wikipedia.org/wiki/Syslog#Facility).
