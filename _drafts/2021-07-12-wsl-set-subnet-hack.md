...
Unfortunately, it has one serious problem: subnet for WSL is selected randomly from all available private subnets, preeferably from `172.16.0.0/12` range. So you turn on your notebook, start WSL, then connect to work VPN and oops - that subnet is already used 🤷‍♂️.
